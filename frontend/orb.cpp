...
void compute_orb(const cv::Mat& img, std::vector<cv::KeyPoint>& keypoints, std::vector<Descriptor>& descriptors){
    ...
    for (const cv::KeyPoint& kp: keypoints){
        ...
        if (center_x < border || center_y < border || center_x >= img.cols - border || center_y >= img.rows - border ){
        ...
        for (int chunk = 0; chunk < ORB_chunks; ++chunk){
            ...
            for (int bit = 0; bit < ORB_chunk_bits; ++bit){
                ...
                const cv::Point2d p1(ORB_bit_pattern_31[pair_idx], ORB_bit_pattern_31[pair_idx + 1]);
                const cv::Point2d p2(ORB_bit_pattern_31[pair_idx + 2], ORB_bit_pattern_31[pair_idx + 3]);
                
                const cv::Point2d p1r(p1.x * cos_theta - p1.y * sin_theta, p1.x * sin_theta + p1.y * cos_theta);
                const cv::Point2d p2r(p2.x * cos_theta - p2.y * sin_theta, p2.x * cos_theta + p2.y * sin_theta);
...
void brute_force_match(const std::vector<Descriptor>& d1, const std::vector<Descriptor>& d2, std::vector<cv::DMatch>& matches){
