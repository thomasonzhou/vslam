...
struct SnavelyReprojectionError {
  SnavelyReprojectionError(const double observed_u, const double observed_v)
  ...
  bool operator()(const T* const camera, const T* const point,
    ...
    const T distortion_correction =
        1.0 + radius_2 * (k1_dist + radius_2 * k2_dist);
  ...
  static ceres::CostFunction* create(const double observed_u,
    ...
    return new ceres::AutoDiffCostFunction<
        SnavelyReprojectionError, util::bal::kPixelDim,
        util::bal::kBALCameraBlockSize, util::bal::kTranslationDim>(observed_u,
                                                                    observed_v);
  }
