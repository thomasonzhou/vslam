...
class VertexIntrinsics
    : public g2o::BaseVertex<util::bal::kIntrinsicsDim, Eigen::Vector3d> {
 public:
...
class VertexPose : public g2o::BaseVertex<util::bal::kPoseDim, Sophus::SE3d> {
 public:
  ...
  void oplusImpl(const double* update) override {
    Eigen::Map<const Eigen::Vector<double, util::bal::kPoseDim>> update_eigen(
        update);
...
class VertexPoint
    : public g2o::BaseVertex<util::bal::kTranslationDim, Eigen::Vector3d> {
 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
  void setToOriginImpl() override { _estimate = Eigen::Vector3d::Zero(); }

  void oplusImpl(const double* update) override {
    Eigen::Map<const Eigen::Vector<double, util::bal::kTranslationDim>>
        update_eigen(update);
    _estimate += update_eigen;
  }
...
class EdgeProjection
    : public g2o::BaseMultiEdge<util::bal::kPixelDim, Eigen::Vector2d> {
 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW;

  EdgeProjection() { resize(3); }

  void computeError() override {
    const VertexIntrinsics* v_k =
        static_cast<const VertexIntrinsics*>(_vertices[kVertexIntrinsicsIdx]);
    const VertexPose* v_pose =
        static_cast<const VertexPose*>(_vertices[kVertexPoseIdx]);
    const VertexPoint* v_point =
        static_cast<const VertexPoint*>(_vertices[kVertexPointIdx]);

    const Eigen::Vector2d pos_pixel =
        v_k->project(v_pose->estimate() * v_point->estimate());
    _error = _measurement - pos_pixel;
  }

  // TODO(thomasonzhou): use explicit derivatives instead of numeric derviatives
