...
void pangolin_draw(const util::BALProblem &bal_problem, const util::BALProblem &bal_problem2){
    pangolin_run([&](){
        glPointSize(2.0f);
        glColor3f(0.0f, 0.0f, 0.0f);
        glBegin(GL_POINTS);
        ...
        for(size_t i = 0; i < std::min(bal_problem.num_points, bal_problem2.num_points); ++i){
