...
class PointUpdateAction : public g2o::HyperGraphAction {
 public:
  PointUpdateAction(const std::vector<VertexType*>& points,
                    std::vector<Eigen::Vector3d>& points_viz,
                    std::mutex& points_mutex, PointReader get_point_fn)
      : points_(points),
        points_viz_(points_viz),
        points_mutex_(points_mutex),
        get_point_fn_(get_point_fn) {}
