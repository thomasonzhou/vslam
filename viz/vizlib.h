...
using ColorMap = std::unordered_map<std::string, ColorTuple>;

inline const ColorMap kColorMap = {
    {"red", {1.0F, 0.0F, 0.0F}},   {"dark_red", {0.5F, 0.0F, 0.0F}},
    {"black", {0.0F, 0.0F, 0.0F}}, {"gray", {0.5F, 0.5F, 0.5F}},
    {"blue", {0.0F, 0.0F, 1.0F}},
};
...
void draw_lines_between(const size_t count1, PointGetter1 get_point1,
                        const size_t count2, PointGetter2 get_point2,
                        const line_config& l_config = line_config{}) {
...
template <typename PointGetter1, typename PointGetter2>
void draw_points_compare(
    const size_t count1, PointGetter1 get_point1, const size_t count2,
    PointGetter2 get_point2,
    const point_compare_config& pcomp_config = point_compare_config{}) {
  ...
  if (pcomp_config.draw_correspondence_lines) {
    draw_lines_between(count1, get_point1, count2, get_point2,
                       pcomp_config.correspondence_config);
  }
}
